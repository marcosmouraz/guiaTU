...
module.exports = class guiaControllers {
    static async createGuia(request, response){
        const {  nome, sobrenome, data_nascimento, cpf, pais, estado, credencial, username, senha_hash } = request.body
        ...
        try {
            ...
            const guia = await Guia.create({
                nome, sobrenome, data_nascimento, cpf, pais, estado, credencial, username, senha_hash: encryptPass
            })

            response.status(201).json({ message: 'Guia cadastrado com SUCESSO!', guia: guia })
        } catch (error) {
